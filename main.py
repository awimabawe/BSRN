...
def check_diagonal_win(cards, selected_indices):
    n = len(cards)
...
def display_bingo_cards(cards, pipe=None):
    try:
        ...
        while True:
            ...
            if key == curses.KEY_UP:
                ...
                if check_diagonal_win(cards, selected_indices):
                    if pipe:
                        win_message = "Diagonal win!"
                        win32file.WriteFile(pipe, win_message.encode())
                    break
            elif key == 27:  # Esc key
                break
